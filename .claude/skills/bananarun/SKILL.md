...
3. ЗАПУСК ИНТЕРФЕЙСА: Сначала найди папку с `run_scraper.py`. Проверь текущую папку, `../virealresearcher`, `~/virealresearcher` и `~/Documents/Банан/competitor parser`. **КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО делать глобальный `find` по диску!** Если не нашел мгновенно — просто спроси путь у пользователя. Затем перейди в папку `cd <путь>` ИЛИ используй абсолютный путь к файлу.
