...
3. ЗАПУСК ИНТЕРФЕЙСА: Получи жесткий путь, который был сохранен при установке. Прочитай файл `~/.banana_parser_path` (сделай `cat ~/.banana_parser_path`). Это и есть АБСОЛЮТНЫЙ ПУТЬ к папке парсера. Если файла нет - значит надо прогнать `/banana-install`. Затем перейди в эту папку `cd <путь>` ИЛИ используй абсолютный путь к файлу.
